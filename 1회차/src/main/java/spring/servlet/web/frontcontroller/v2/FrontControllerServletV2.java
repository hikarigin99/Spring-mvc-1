...
public class FrontControllerServletV2 extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ControllerV2 controller = controllerMap.get(requestURI);    // 다형성 부모 - 자식 (ControllerV1 .. = MemberListControllerV1)
        if(controller == null) {
            response.setStatus(HttpServletResponse.SC_NOT_FOUND);   // 404 오류
            return;
        }
        MyView view = controller.process(request, response);// 다형성으로 인해 오버라이드 된 process 메서드 호출
